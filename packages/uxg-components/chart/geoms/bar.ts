  ...
  return (source: Observable<ChartModel>) => {
    return source.pipe(
      mergeMap(data => {
        ...
        const barTemplate: Template = rectangle({ x: 0, y: 0, width: 50, height: 50, radius: 10 });
        const barTemplate1: Template = rectangle({ x: 110, y: 0, width: 50, height: 50 });
        const barTemplate2: Template = rectangle({ x: 110, y: 110, width: -50, height: 50, radius: 10 });

        const barTemplate3: Template = rectangle({ x: 200, y: 210, width: -50, height: -50, radius: 10 });

        const barTemplate4: Template = rectangle({ x: 100, y: 310, width: 50, height: -50, radius: 10 });

        data.templateTree.children = data.templateTree.children || [];
        data.templateTree.children.push(barTemplate, barTemplate1, barTemplate2, barTemplate3, barTemplate4);
        return of(data);
      })
    );
  };
