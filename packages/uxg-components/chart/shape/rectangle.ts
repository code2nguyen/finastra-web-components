...
function getRectanglePath(x: number, y: number, width: number, height: number, radius?: number | RectRadius): SVGPath {
  ...
  if (!radius) {
    paths = [{ c: 'M', x: startX, y: startY }, { c: 'h', x: w }, { c: 'v', y: h }, { c: 'h', x: -w }, { c: 'Z' }];
  } else {
...
export function rectangle({ x, y, width, height, radius, className, ...others }: RectangleProps): Template {
